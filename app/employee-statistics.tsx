...
export default function EmployeeStatisticsScreen() {
  ...
  const loadStatistics = async () => {
    ...
    try {
      ...
      const [closedData, openData] = await Promise.all([
        getClosedResponses(employeeId),
        getOpenResponses(employeeId)
      ]);
      ...
      // Filtrar por período
      const filteredResponses = filterByTimePeriod(allResponses, selectedFilter);
  ...
  const filterByTimePeriod = (responses: any[], filter: TimeFilter) => {
    const now = new Date();
    let startDate: Date;
  ...
  useEffect(() => {
    loadStatistics();
  }, [selectedFilter, selectedService.id, employeeId]);
